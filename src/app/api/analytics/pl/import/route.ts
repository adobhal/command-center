...
/**
 * Import P&L insights from a file path (for server-side imports)
 */
export async function POST(request: Request) {
  try {
    ...
    for (let yearIdx = 1; yearIdx <= 6; yearIdx++) {
      ...
      const plData = parser.parseData(yearData as any[][], `${filePath} - ${year}`, 1);
    ...
    logger.info('P&L insights imported', {
      filePath,
